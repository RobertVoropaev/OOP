...
#include <string>
using namespace std;
class Email_not_found : public exception {
} email_not_found;
template <typename T>
...
public:
	person_data() {}
	person_data(string phone) { phones.push_back(phone); }
	friend class Book;
};
class Book {
...
public:
	...
	void print_phones(string fio) {
		...
		for (j = phones.begin(); j != phones.end(); ++j) cout << *j << endl;
	}
	...
	while (true) {
		...
		if (s == "end") break;
			...
			if (!b.exists(fio))
				...
				b.print_phones(fio);
			q: cout << "replace_all / add_new / cancel";
				string c;
				cin >> c;
				if (c == "replace_all") b.set_phone(fio, phone);
				else if (c == "add_new") b.add_phone(fio, phone);
				else goto q;
			}
		}
		else if (s == "delete") {
			...
			catch (out_of_range &) { cout << "fio not found\n"; }
		}
			...
			catch (out_of_range &) { cout << "fio/phone not found\n"; }
		}
		else if (s == "print_phones") {
			string fio;
			cin.get();
			getline(cin, fio);
			try { b.print_phones(fio); }
			catch (out_of_range &) { cout << "fio not found\n"; }
		}
			...
			try {
				...
				if (email != "") cout << email << endl;
			}
			catch (out_of_range &) { cout << "fio not found\n"; }
		}
		else if (s == "print_all_fio") b.print_all_fio();
		else if (s == "print_all_phones") b.print_phones();
		else if (s == "who") {
			string phone;
			cin >> phone;
			...
			cin.get();
			getline(cin, fio);
			try { cout << b.get_email(fio); }
			catch (out_of_range &) { cout << "fio not found\n"; }
			catch (Email_not_found &) { cout << "email not found\n"; }
		}
		else if (s == "help" || s == "?") {
			cout
				<< "end\n"
		...
		else cout << "Unknown command. Type 'help' to see all commands\n";
	}
	return 0;
}